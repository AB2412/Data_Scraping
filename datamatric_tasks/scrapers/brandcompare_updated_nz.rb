...
class BrandcompareUpdatedNz
  ...
  def search_provider(cookie_value, id, search_query, page_number, flag, provider_id, retries = 10)
    begin
      ignore_address = "&f%5Bignore_address_coverage%5D=1"

      uri = URI.parse("https://www.broadbandcompare.co.nz/plan/search/index?f%5Bsave_to_session%5D=1&f%5Btracking_enabled%5D=1#{ignore_address}&f%5Baddress%5D%5Bformatted%5D=#{search_query}&f%5Baddress%5D%5Bid%5D=#{id}&f%5Badvanced_filters%5D%5Bprovider_ids%5D%5B%5D=#{provider_id}&f%5Bsort%5D=rating&f%5Bprice_type%5D=per_month&page=#{page_number}&per-page=50")
      ...
      data_request(cookie_value, page_number, flag, provider_id)
    end
  end
  ...
  def main
    ...
    excluded_providers = ["Vodafone", "Spark", "2degrees", "Sky", "Skinny", "Slingshot"]
    ...
    (1..200).each do |source_provider_id|
      puts "Source provider ID ->  #{source_provider_id}"
      response = search_provider(cookie_value, id, search_query, page_number, flag, source_provider_id)
      ...
      while page_number <= total_pages
        ...
        response = search_provider(cookie_value, id, search_query, page_number, flag, source_provider_id)
        data = JSON.parse(response.body)
        json_current_page = data['meta']['current_page']
        redo if page_number - 1 != json_current_page
        ...
        all_plans.each_with_index do |plan, index|
          ...
          available_contracts.each_with_index do |contract, contract_index|
            ...
            if plans_data_hash[:connection_type] == ""
              ...
              plans_data_hash[:connection_type] = "RBI" if plans_data_hash[:name].downcase.include? 'rbi' or plans_data_hash[:name].downcase.include? 'rural'
            end
            ...
            if plans_data_hash[:connection_type] == ''
              if plans_data_hash[:byo_modem] != '' or plans_data_hash[:rental_router] != ''
                plans_data_hash[:connection_type] = "Wireless"
              end
            end
