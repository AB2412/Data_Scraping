...
class FinderMobile
  ...
  def data_processing(hash_array, page, available_plans, index, driver, country_id, all_brands, usage_type)
    data_hash = {}
    providers_data_hash = {}
    ...
    price = driver.find_element(css: 'form[name="compareForm"]').find_elements(css: 'tbody tr')[index].find_element(css: 'div[data-heading="Price"]').find_element(css: "div[class='price-main']").text
    details_text_price = driver.find_element(css: 'form[name="compareForm"]').find_elements(css: 'tbody tr')[index].find_elements(css: 'div.price')[0].text.squish
    data_hash["price"] = price
    data_hash["price_details"] = details_text_price
    ...
    network_details = available_plans[index].css('td.comparison-table__product ul.list-1 li')[1].text
...
#NEW entertainment_%
    ...
    data_hash["promotions"] = available_plans[index].css('div.offer-details')[0].text.strip rescue ''
...
#<-->
  ...
  def scrape_using_usage(page_num, total_pages, driver, pp, all_brands, country_id, hash_array, usage_type, last_page_visited, current_date)
    ...
    while page_num <= total_pages
      ...
      if page_num >= last_page_visited
        ...
        if usage_type == "Business"
          available_plans = pp.css('form[name="compareForm"]')[0].css('table tbody tr')
        else
        ...
        1.times.map {
          Thread.new(product_ids) do |records|
            while product = mutex.synchronize { records.pop }
              ...
              provider_name = get_values(page, "Provider name")
              hash_array = data_processing(hash_array, page, available_plans, index, driver, country_id, all_brands, usage_type)
        ...
        if usage_type == "Personal"
          AustraliaMobileStat.where(run_date: current_date).update(personal_page: page_num)
        else
          AustraliaMobileStat.where(run_date: current_date).update(business_page: page_num)
        end
        hash_array = []
        page_num += 1
        button = driver.find_elements(css: 'button.comparison-table__paginationNav-next')[0]
  ...
  def scraper
    ...
    last_run_personal = AustraliaMobileStat.pluck(:personal_run).last
    last_run_business = AustraliaMobileStat.pluck(:business_run).last

    hash_array = []
    (0..1).each do |run|
      if run == 0
        if last_run_personal == false
          last_page_visited = AustraliaMobileStat.pluck(:personal_page).last
          pp = Nokogiri::HTML(driver.page_source)
          ...
          if last_page_visited == 0
            AutraliaMobilePlan.where(source_url: 'https://www.finder.com.au/mobile-plans',  usage_type: 'Personal').update_all('is_visited': false)
          end
          hash_array = scrape_using_usage(page_num, total_pages, driver, pp, all_brands, country_id, hash_array, 'Personal', last_page_visited, current_date)
          AutraliaMobilePlan.where(source_url: 'https://www.finder.com.au/mobile-plans', usage_type: 'Personal', is_visited: false).update_all(is_deleted: true)
          AustraliaMobileStat.where(run_date: current_date).update(personal_run: true)
        end
      else
        if last_run_business == false
          last_page_visited = AustraliaMobileStat.pluck(:business_page).last
          driver.find_elements(css: "span").select{|e| e.text == 'Types'}[0].click()
          sleep(2)
          driver.find_elements(css: "span").select{|e| e.text == 'Business mobile plans'}[0].click()
          sleep(3)
          driver.find_elements(css: '.luna-label').select{ |e| e.text == 'Only show products with direct links & enquiry options'}[0].click()
          sleep(3)
          pp = Nokogiri::HTML(driver.page_source)
          total_pages = pp.css('.comparison-table__paginationContainer').css(".luna-buttonGroup.comparison-table__paginationNav-numbers")[0].css(".luna-button").last.text.strip.to_i        # total_pages = (total_values/10).to_i
          page_num = 0
          if last_page_visited == 0
            AutraliaMobilePlan.where(source_url: 'https://www.finder.com.au/mobile-plans',  usage_type: 'Business').update_all('is_visited': false)
          end
          hash_array = scrape_using_usage(page_num, total_pages, driver, pp, all_brands, country_id, hash_array, 'Business', last_page_visited, current_date)
          AutraliaMobilePlan.where(source_url: 'https://www.finder.com.au/mobile-plans', usage_type: 'Business', is_visited: false).update_all(is_deleted: true)
          AustraliaMobileStat.where(run_date: current_date).update(business_run: true)
        end
      end
    end
