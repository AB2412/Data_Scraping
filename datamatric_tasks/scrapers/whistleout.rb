...
class Whistleout
  ...
  def download(download_speed, features_array)
    return nil if download_speed.nil?
  ...
  def scraper()
    ...
    tabs.each_with_index do |tab, tab_index_no|
      usage_type.each do |type|
        ...
        if tab == 'homewireless'
          total_records = pp.css('.results-list .results-item')
        elsif tab == 'other'
          total_records = total_records = pp.css('.results-list .results-item')
          total_records = []
        else
          total_records = total_records = pp.css('.results-list .results-item')
          total_records = []
        ...
        total_records.each_with_index do |data, index|
          puts " I was here  #{index}" 
          xx = data.css('span.label-warning')[0].text.squish == 'Ad' rescue nil
          next unless xx.nil?
          main_page_url =  data.css('a').select { |e| e.text.squish == 'Plan T&Cs'}
          next if main_page_url.empty?
          complete_link = 'https://www.whistleout.com.au/' + main_page_url[0]['href']   
          data_hash = {}
          search_data_hash = {}
          providers_data_hash = {}
          ...
          # 
          # data_hash['features'] = total_records[index].text.squish
          data_hash['features'] = total_records[index].text.split("\n").map { |e| e.squish}.reject { |e| e.empty? }.join("\n")
          # features_array = total_records[index].text.split("\n")
          features_array =  total_records[index].text.split("\n").map { |e| e.squish}.reject { |e| e.empty? }

          download_speed = all_dowload_speeds[index]
          if tab == "other"
            download_speed = '' if download_speed.nil?
          ...
          if data_hash['data_details'].downcase.include? 'peak'
            ...
            data_hash['peak_data_times'] = splited_array.select{|e| e.include? ' peak'}[0].split('(')[-1].gsub(')','').strip rescue nil
          else
          ...
          if data_hash['is_on_promotion'] == 'Yes'
            if data_hash['promotions'].include? 'Enjoy' and data_hash['promotions'].include? 'off this'
              ...
              new_ind = data_hash['promotions'].index arr[1] rescue nil
              data_hash['price'] = data_hash['promotions'][ind..new_ind].split.select{|e| e.include? '$'}[0].gsub('/mth','')  rescue nil
            else
