...
class Parser < Hamster::Parser
  ...
  def parse_data(profile_html,section_html,education_html,license_html,category_html,run_id,domain)
    ...
    data_hash[:bar_number]               = get_general_info(infor[0], 'registrationNumber')
    ...
    data_hash[:licenses]                 = (infor.count > 1) ? get_licenses_array(infor, domain) : nil
    data_hash[:md5_hash]                 = create_md5_hash(data_hash)
    data_hash[:practice_areas]           = get_general_info(parsed_profile_html,'categories')
  ...
  def get_licenses_array(array, domain)
    license_array = []
    array[1..].each do |row|
      row_hash = {}
      row_hash[:bar_number]          = get_general_info(row , 'registrationNumber')
      row_hash[:state_admission]     = get_general_info(row , 'stateAdmission')
      full_date   = get_general_info(row , 'yearAdmission').split("T").first
      final_date = (full_date.include? "/") ?  DateTime.strptime(full_date, '%m/%d/%Y').to_date: DateTime.strptime(full_date.gsub("-","/"), '%m/%d/%Y').to_date rescue nil
      date = (final_date.nil?) ? DateTime.strptime(full_date.gsub("-","/"), '%Y/%m/%d').to_date : final_date
      row_hash[:date_admitted]       = date
      row_hash[:registration_status] = get_general_info(row , 'statusName')
      license_array << row_hash
    end
    license_array.to_json
  end
